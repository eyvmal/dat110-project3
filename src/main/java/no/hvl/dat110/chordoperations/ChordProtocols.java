	...
	public void fixFingerTable() {
		...
		try {
			...
			for (int i = 0; i < bits; i++) {
				...
				BigInteger key = chordnode.getNodeID().add(BigInteger.valueOf(2).pow(i)).mod(addressSize);

				// then: use chordnode to find the successor of k. (i.e., succnode = chordnode.findSuccessor(k))
				NodeInterface nextNode = chordnode.findSuccessor(key);

				// check that succnode is not null, then add it to the finger table
				if (nextNode != null) {
					try {
						fingerTable.set(i, nextNode);
					} catch (IndexOutOfBoundsException e) {
						fingerTable.add(i, nextNode);
					}
				}
			}
		} catch (RemoteException e) {
